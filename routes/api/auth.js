...
router.post('/signup', authValidation, asyncWrapper(AuthController.registrationCtrl))

router.post('/login', authValidation, asyncWrapper(AuthController.loginCtrl))

router.post('/logout/:userId', asyncWrapper(AuthController.logoutCtrl))

router.get('/current', authMiddleware, asyncWrapper(AuthController.findCurrentUserCtrl))
